...
void Cholesky::initGPUMemory() {
    if (conf::unifiedAddressSpace == false) {
        // init GPU data structure for matrix A on which the Cholesky decomposition should be
        // performed
...
void Cholesky::shiftSplit(const int blockCountATotal, const std::size_t blockSizeBytes, const int k,
    ...
    if (k % loadBalancer->updateInterval == 0 && k != 0 && gpuProportion != 0 &&
        ...
        if (gpuProportion_new == 1 && k < A.blockCountXY - minBlockCountGPU &&
            ...
            // if GPU proportion turns 100%, current diagonal block might now be needed on the GPU
            // if min GPU block count not yet reached
            gpuQueue
                .submit([&](handler &h) {
                    h.memcpy(&A_gpu[blockStartIndexDiagBlock],
                             &A.matrixData[blockStartIndexDiagBlock], blockSizeBytes);
                })
                .wait();
        }
    }
    ...
    const int blockCountGPU_new =
        std::min(std::max(static_cast<int>(std::ceil(blockCountColumn * gpuProportion_new)),
                          minBlockCountGPU),
                 blockCountColumn);
    ...
    if (gpuProportion != 0 && gpuProportion != 1) {
        ...
        if (conf::unifiedAddressSpace == false) {
            if (blockCountGPU_new >= minBlockCountGPU) {
                // re-balancing: move split up or down by possibly multiple rows, depending on new
                ...
                if (blockStartGPU_new < blockStartGPU) {
                    ...
                    // copy part of each column that was computed on the CPU and now has to be
                    // computed by the GPU after re-balancing
                    for (int c = 0; c < blockCountCPU_new + k + 1 + additionalBlocks; ++c) {
                        ...
                        gpuQueue.submit([&](handler &h) {
                            h.memcpy(&A_gpu[blockStartIndexFirstGPUBlock],
                                     &A.matrixData[blockStartIndexFirstGPUBlock],
                                     blockSizeBytes * blocksToCopy);
                        });
                    }
                    gpuQueue.wait();
                    ...
                    // copy part of each column that was computed on the GPU and now has to be
                    // computed by the CPU after re-balancing
...
void Cholesky::choleskyUpdateCurrentDiagonalBlock(const std::size_t blockSizeBytes, const int k,
                                                  const int blockID,
                                                  const std::size_t blockStartIndexDiagBlock) {
    executionTimes.startCholesky = std::chrono::steady_clock::now();
    if ((k < A.blockCountXY - std::max(blockCountGPU, minBlockCountGPU) && gpuProportion != 1) ||
        ...
        if (gpuProportion != 1 && conf::unifiedAddressSpace == false) {
            ...
            gpuQueue
                .submit([&](handler &h) {
                    h.memcpy(&A.matrixData[blockStartIndexDiagBlock],
                             &A_gpu[blockStartIndexDiagBlock], blockSizeBytes);
                })
                .wait();
        }
    }
...
void Cholesky::choleskySolveTriangularSystemColumn(const std::size_t blockSizeBytes, const int k,
                                                   const int blockID) {
    executionTimes.startTriangularSolve = std::chrono::steady_clock::now();
    if (blockCountCPU > 0) {
        executionTimes.eventCPU_triangularSolve =
            MatrixMatrixOperations::triangularSolve_optimizedCPU(cpuQueue, A.matrixData.data(),
                                                                 blockID, k, k + 1, blockCountCPU);
    }
    if (blockCountGPU > 0) {
        executionTimes.eventGPU_triangularSolve =
            MatrixMatrixOperations::triangularSolve_optimizedGPU(gpuQueue, A_gpu, blockID, k,
    ...
    if (gpuProportion != 1 && gpuProportion != 0 && blockCountCPU > 0 &&
        ...
        const std::size_t blockStartIndexFirstCPUSystem =
            static_cast<std::size_t>(blockID + 1) * conf::matrixBlockSize * conf::matrixBlockSize;
    ...
    if (blockCountCPU > 0) {
        executionTimes.eventCPU_matrixMatrixDiag =
            MatrixMatrixOperations::symmetricMatrixMatrixDiagonal_optimizedCPU(
                cpuQueue, A.matrixData.data(), blockID, k, k + 1, blockCountCPU, A.blockCountXY);
    }
    if (blockCountGPU > 0) {
        executionTimes.eventGPU_matrixMatrixDiag =
            MatrixMatrixOperations::symmetricMatrixMatrixDiagonal_optimizedGPU(
                gpuQueue, A_gpu, blockID, k, blockStartGPU, blockCountGPU, A.blockCountXY);
    }
    waitAllQueues();
...
void Cholesky::choleskyUpdateLowerBlockTriangle(const int k, const int blockID) {
    ...
    if (blockCountCPU > 1) {
        ...
        case 0:
            executionTimes.eventCPU_matrixMatrix =
                MatrixMatrixOperations::matrixMatrixStep(cpuQueue, A.matrixData.data(), blockID, k,
                                                         k + 2, blockCountCPU - 1, A.blockCountXY);
            break;
        case 1:
            executionTimes.eventCPU_matrixMatrix =
                MatrixMatrixOperations::matrixMatrixStep_optimizedCPU(
                    cpuQueue, A.matrixData.data(), blockID, k, k + 2, blockCountCPU - 1,
                    A.blockCountXY);
            break;
        case 2:
            executionTimes.eventCPU_matrixMatrix =
    ...
    if (blockCountGPU > 1) {
        ...
        case 0:
            executionTimes.eventGPU_matrixMatrix = MatrixMatrixOperations::matrixMatrixStep(
                gpuQueue, A_gpu, blockID, k, blockStartGPU + offsetMatrixMatrixStepGPU,
...
void Cholesky::printTimes(const int k) {
    ...
    const auto copyTime_row = std::chrono::duration<double, std::milli>(
                                  executionTimes.endCopy_row - executionTimes.startCopy_row)
                                  .count();
    const auto copyTime_column =
        std::chrono::duration<double, std::milli>(executionTimes.endCopy_column -
            ...
            .count();
    const auto choleskyTime = std::chrono::duration<double, std::milli>(
                                  executionTimes.endCholesky - executionTimes.startCholesky)
                                  .count();
    ...
    if (conf::printVerbose) {
        std::cout << "   -- matrix-matrix diagonal: " << matrixMatrixDiagonalTime << "ms"
                  << std::endl;
    }
    if (blockCountCPU > 0 && !intel) {
...
void Cholesky::printFinalTimes() {
    ...
    const auto totalTime =
        std::chrono::duration<double, std::milli>(executionTimes.end - executionTimes.start)
            .count();
...
void Cholesky::solve_heterogeneous() {
    ...
    const std::size_t blockSizeBytes =
        conf::matrixBlockSize * conf::matrixBlockSize * sizeof(conf::fp_type);
    ...
    // Calculate initial block-count values for CPU and GPU. These values correspond to the number
    // of row-blocks
    ...
    for (int k = 0; k < A.blockCountXY; ++k) {
        ...
        // check if row that splits GPU/CPU part of the matrix has to change and apply the change if
        // necessary
        shiftSplit(blockCountATotal, blockSizeBytes, k, blockStartIndexDiagBlock);

        // change offset for matrix-matrix step from 0 to 1 depending on how far we are into the
        // computation
