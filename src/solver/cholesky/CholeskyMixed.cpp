            ...
            gpuQueue
                .submit([&](handler &h) {
                    h.memcpy(A_gpu + blockOffsetDiagBlock,
                             A.matrixData.data() + blockOffsetDiagBlock, blockSizeBytes);
                })
                .wait();
        }
    }
...
void CholeskyMixed::choleskyUpdateCurrentDiagonalBlock(const std::size_t blockSizeBytes,
    ...
    if ((k < A.blockCountXY - std::max(blockCountGPU, minBlockCountGPU) && gpuProportion != 1) ||
        ...
        case 2:
            MatrixOperationsMixed::cholesky(cpuQueue,
                                            reinterpret_cast<sycl::half *>(A.matrixData.data()),
                                            blockOffsetDiagBlock, k);
            break;
        case 4:
            MatrixOperationsMixed::cholesky(
                cpuQueue, reinterpret_cast<float *>(A.matrixData.data()), blockOffsetDiagBlock, k);
            break;
        case 8:
            MatrixOperationsMixed::cholesky(
                cpuQueue, reinterpret_cast<double *>(A.matrixData.data()), blockOffsetDiagBlock, k);
            break;
        };
        cpuQueue.wait();
        ...
        case 2:
            MatrixOperationsMixed::cholesky_optimizedGPU(
                gpuQueue, reinterpret_cast<sycl::half *>(A_gpu), blockOffsetDiagBlock, k);
            break;
        case 4:
...
void CholeskyMixed::copyResultFromGPU(const int blockCountATotal) {
    ...
    if (gpuProportion != 1 && gpuProportion != 0) {
        ...
        for (int k = 0; k < A.blockCountXY; ++k) {
            ...
            gpuQueue.submit([&](handler &h) {
                h.memcpy(A.matrixData.data() + blockStartOffset, A_gpu + blockStartOffset,
                         blockByteCount);
            });
        }
        gpuQueue.wait();
        ...
        gpuQueue.submit([&](handler &h) { h.memcpy(A.matrixData.data(), A_gpu, totalBytes); })
            .wait();
    }
