...
class Cholesky {
  public:
    ...
    void writeMetricsToFile(); /// writes the json file generated by the metrics tracker with all
                               /// runtime metrics
  ...
  private:
    ...
    /**
     * This operation shifts the horizontal split between the CPU and GPU up or down depending on
     * the new desired GPU proportion. It ensures consistency of the matrix across the CPU and GPU
     * after split is shifted.
     */
    void shiftSplit(int blockCountATotal, std::size_t blockSizeBytes, int k,
                    std::size_t blockStartIndexDiagBlock);

    /**
     * Performs the first step of a column step of the Cholesky decomposition: a small Cholesky
     * decomposition of the current diagonal block.
     *
     ...
     */
    void choleskyUpdateCurrentDiagonalBlock(std::size_t blockSizeBytes, int k, int blockID,
                                            std::size_t blockStartIndexDiagBlock);

    /**
     * Performs the second step of a column step of the Cholesky decomposition: updating the column
     * below the diagonal block by solving a matrix equation.
     *
     * @param blockSizeBytes size of one block in bytes
     * @param k column k
     * @param blockID id of the diagonal block
    ...
    /**
     * Performs the third step of a column step of the Cholesky decomposition: updating the
     * remaining diagonal blocks using symmetric matrix-matrix multiplications
     *
     * @param k column k
     * @param blockID id of the diagonal block
     */
    ...
    /**
     * Performs the third step of a column step of the Cholesky decomposition: updating the
     * remaining non-diagonal blocks using matrix-matrix multiplications.
     *
     * @param k
     * @param blockID
     */
    ...
    /**
     * This operation copies the part of the final lower triangular matrix computed on the GPU to
     * the CPU after the decomposition
     *
