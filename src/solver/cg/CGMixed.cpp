...
CGMixed::CGMixed(SymmetricMatrixMixed &A, RightHandSide &b, queue &cpuQueue, queue &gpuQueue,
...
void CGMixed::initGPUdataStructures() {
    ...
    for (std::size_t i = 0; i < A.precisionTypes.size(); ++i) {
        currentBlockEnd = A.precisionTypes[i] * A.blockSize * A.blockSize + A.blockByteOffsets[i];
    ...
    const std::size_t totalBlockCountA = (A.blockCountXY * (A.blockCountXY + 1) / 2);
    ...
    if (maxBlocksGPUMemory < totalBlockCountA) {
        ...
        if (valueRoot >= 0.0) {
            ...
            for (std::size_t block = 0; block < blocksGPUMemory; block++) {
                bytesGPU += A.precisionTypes[block] * conf::matrixBlockSize * conf::matrixBlockSize;
            }
...
void CGMixed::initCG(conf::fp_type &delta_zero, conf::fp_type &delta_new) {
    ...
    if (blockCountGPU != 0) {
        MatrixVectorOperationsMixed::matrixVectorBlock(
            gpuQueue, A_gpu, x_gpu, r_gpu, A.precisionTypes.data(), A.blockByteOffsets.data(), 0, 0,
            blockCountGPU, A.blockCountXY, A.blockCountXY);
    }
    if (blockCountCPU != 0) {
        MatrixVectorOperationsMixed::matrixVectorBlock(
            cpuQueue, A.matrixData.data(), x.data(), r_cpu, A.precisionTypes.data(),
            A.blockByteOffsets.data(), blockStartCPU, 0, blockCountCPU, A.blockCountXY,
            A.blockCountXY);
    }
    waitAllQueues();
    ...
    if (blockCountGPU != 0) {
        if (conf::gpuOptimizationLevel == 0) {
            eventGPU = MatrixVectorOperationsMixed::matrixVectorBlock(
                gpuQueue, A_gpu, d_gpu, q_gpu, A.precisionTypes.data(), A.blockByteOffsets.data(),
