...
CG::CG(SymmetricMatrix &A, RightHandSide &b, queue &cpuQueue, queue &gpuQueue,
    ...
    if (A.N != b.N) {
        throw std::invalid_argument("Dimensions of A and b do not match: " + std::to_string(A.N) +
                                    " != " + std::to_string(b.N));
    }
...
void CG::solveHeterogeneous() {
    ...
    while (iteration < conf::iMax && delta_new > epsilon2 * delta_zero) {
        ...
        auto iterationTime =
            std::chrono::duration<double, std::milli>(endIteration - startIteration).count();
        ...
        auto time_alpha =
            std::chrono::duration<double, std::milli>(timePoint3 - timePoint2).count();
        ...
        auto time_delta =
            std::chrono::duration<double, std::milli>(timePoint6 - timePoint5).count();
    ...
    if (blockCountGPU != 0 && conf::unifiedAddressSpace == false) {
        ...
        metricsTracker.resultCopyTime =
            std::chrono::duration<double, std::milli>(endMemCopy - startMemCopy).count();
    }
...
void CG::initGPUdataStructures() {
    ...
    const std::size_t gpuAvailableMemorySize =
        0.9 * gpuQueue.get_device().get_info<sycl::info::device::global_mem_size>() -
        6 * b.rightHandSideData.size() * sizeof(conf::fp_type);

    const double maxBlocksGPUMemory =
        std::floor(static_cast<double>(gpuAvailableMemorySize) /
                   (static_cast<double>(conf::matrixBlockSize) *
                    static_cast<double>(conf::matrixBlockSize) * sizeof(conf::fp_type)));
...
void CG::initCG(conf::fp_type &delta_zero, conf::fp_type &delta_new) {
    // r = b - Ax
    if (blockCountGPU != 0) {
        MatrixVectorOperations::matrixVectorBlock(gpuQueue, A_gpu, x_gpu, r_gpu, 0, 0,
    ...
    if (blockCountCPU != 0) {
        MatrixVectorOperations::matrixVectorBlock(cpuQueue, A.matrixData.data(), x.data(), r_cpu,
                                                  blockStartCPU, 0, blockCountCPU, A.blockCountXY,
                                                  A.blockCountXY);
    }
    waitAllQueues();
    ...
    if (blockCountCPU != 0) {
        VectorOperations::sumFinalScalarProduct_CPU(cpuQueue, tmp_cpu,
    ...
    if (blockCountGPU != 0) {
        ...
        gpuQueue.submit([&](handler &h) { h.memcpy(&delta_new, tmp_gpu, sizeof(conf::fp_type)); })
            .wait();
    }
    if (blockCountCPU != 0) {
...
void CG::compute_q() {
    ...
    if (blockCountGPU != 0) {
        if (conf::gpuOptimizationLevel == 0) {
            eventGPU = MatrixVectorOperations::matrixVectorBlock(
                gpuQueue, A_gpu, d_gpu, q_gpu, 0, 0, blockCountGPU, A.blockCountXY, A.blockCountXY);
        } else {
    ...
    if (blockCountCPU != 0) {
        if (conf::cpuOptimizationLevel == 0) {
            eventCPU = MatrixVectorOperations::matrixVectorBlock(
                cpuQueue, A.matrixData.data(), d_cpu, q_cpu, blockStartCPU, 0, blockCountCPU,
    ...
    if (blockCountGPU != 0 && !intel) {
        metricsTracker.matrixVectorTimes_GPU.push_back(
            static_cast<double>(
                eventGPU.get_profiling_info<sycl::info::event_profiling::command_end>() -
                eventGPU.get_profiling_info<sycl::info::event_profiling::command_start>()) /
            1.0e6);
    } else {
    ...
    if (blockCountCPU != 0 && !intel) {
        metricsTracker.matrixVectorTimes_CPU.push_back(
            static_cast<double>(
                eventCPU.get_profiling_info<sycl::info::event_profiling::command_end>() -
                eventCPU.get_profiling_info<sycl::info::event_profiling::command_start>()) /
            1.0e6);
    } else {
...
void CG::compute_alpha(conf::fp_type &alpha, conf::fp_type &delta_new) {
    ...
    if (blockCountGPU != 0) {
        gpuQueue.submit([&](handler &h) { h.memcpy(&result, tmp_gpu, sizeof(conf::fp_type)); })
            .wait();
    }
    if (blockCountCPU != 0) {
...
void CG::update_x(conf::fp_type alpha) {
    // x = x + 𝛼d
    if (blockCountGPU != 0) {
        VectorOperations::scaleAndAddVectorBlock(gpuQueue, x_gpu, alpha, d_gpu, x_gpu, 0,
                                                 blockCountGPU);
    }
    if (blockCountCPU != 0) {
        VectorOperations::scaleAndAddVectorBlock(cpuQueue, x.data(), alpha, d_cpu, x.data(),
                                                 blockStartCPU, blockCountCPU);
    }
    waitAllQueues();
}
...
void CG::computeRealResidual() {
    ...
    if (blockCountGPU != 0) {
        MatrixVectorOperations::matrixVectorBlock_GPU(
            gpuQueue, A_gpu, x_gpu, r_gpu, 0, 0, blockCountGPU, A.blockCountXY, A.blockCountXY);
    }
    if (blockCountCPU != 0) {
        MatrixVectorOperations::matrixVectorBlock_CPU(cpuQueue, A.matrixData.data(), x.data(),
                                                      r_cpu, blockStartCPU, 0, blockCountCPU,
                                                      A.blockCountXY, A.blockCountXY);
    }
    waitAllQueues();
...
void CG::update_r(conf::fp_type alpha) {
    // r = r - 𝛼q
    if (blockCountGPU != 0) {
        VectorOperations::scaleAndAddVectorBlock(gpuQueue, r_gpu, -1.0 * alpha, q_gpu, r_gpu, 0,
                                                 blockCountGPU);
    }
    if (blockCountCPU != 0) {
        VectorOperations::scaleAndAddVectorBlock(cpuQueue, r_cpu, -1.0 * alpha, q_cpu, r_cpu,
                                                 blockStartCPU, blockCountCPU);
    }
    waitAllQueues();
}
...
void CG::compute_d(conf::fp_type &beta) {
    // d = r + βd
    if (blockCountGPU != 0) {
        VectorOperations::scaleAndAddVectorBlock(gpuQueue, r_gpu, beta, d_gpu, d_gpu, 0,
                                                 blockCountGPU);
    }
    if (blockCountCPU != 0) {
        VectorOperations::scaleAndAddVectorBlock(cpuQueue, r_cpu, beta, d_cpu, d_cpu, blockStartCPU,
                                                 blockCountCPU);
    }
    waitAllQueues();
}
