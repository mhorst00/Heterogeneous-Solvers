#ifndef MATRIXOPERATIONSMIXED_HPP
#define MATRIXOPERATIONSMIXED_HPP

#include <sycl/sycl.hpp>

class MatrixOperationsMixed {
public:
  /**
   ...
   * @param A complete matrix, on which the overall blocked Cholesky is
   * performed
   * @param blockID ID of the diagonal block on which this method should perform
   * a decomposition
  ...
  /**
   * This method performs a cholesky decomposition on a diagonal block of the
   * matrix A. The kernel launched by this method is optimized for the execution
   * on GPUs. It makes use of local memory.
   *
   * The method launches a kernel on the device asynchronously and returns after
   * that. One has to wait to ensure correctness.
   *
   ...
   * This method performs a cholesky decomposition on a diagonal block of the
   * matrix A. The kernel launched by this method is optimized for the execution
   * on GPUs. It uses local memory and additional work-items to set the upper
   * diagonal to zero.
   *
   * The method launches a kernel on the device asynchronously and returns after
   * that. One has to wait to ensure correctness.
...
#endif // MATRIXOPERATIONSMIXED_HPP
