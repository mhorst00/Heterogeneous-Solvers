            ...
            for (int k = 0; k < static_cast<int>(matrixBlockSize); ++k) {
                ...
                if (local_i > k) {
                    ...
                    conf::fp_type val =
                        ...
                        read_element(ABytes, blockStartOffset_L, local_i * matrixBlockSize + k,
                                     blockPrec) *
                            b_k;
                    write_element(ABytes, blockStartOffset_B,
                                  group_id_j * matrixBlockSize + local_i, val, blockPrec_B);
                }
    ...
    sycl::event event = queue.submit([&](sycl::handler &h) {
        ...
        h.parallel_for(kernelRange, [=](auto &nd_item) {
            ...
            for (int k = 0; k < static_cast<int>(matrixBlockSize); ++k) {
                if (local_i > k) {
                    // b_i = b_i - a_ik*b_k
                    value = value - read_element(ABytes, blockStartOffset_L,
...
sycl::event MatrixMatrixOperationsMixed::symmetricMatrixMatrixDiagonal_optimizedGPU(
    ...
    sycl::event event = queue.submit([&](sycl::handler &h) {
        ...
        h.parallel_for(kernelRange, [=](auto &nd_item) {
            ...
            if (i >= j) {
                value = read_element(ABytes, block_wg_diag_offset, i * matrixBlockSize + j,
                                     block_wg_diag_prec);
            }
            ...
            for (int t = 0; t < static_cast<int>(matrixBlockSize) / wgSize_xy; ++t) {
                ...
                if (i >= j || group_mod_count_i == group_mod_count_j) {
                    ...
                    local_tile_A[local_i][local_j] = read_element(
                        ABytes, block_wg_col_offset, i * matrixBlockSize + t * wgSize_xy + local_j,
                        block_wg_col_prec);

                    // transposed block
                    local_tile_B[local_i][local_j] = read_element(
                        ABytes, block_wg_col_offset, j * matrixBlockSize + t * wgSize_xy + local_i,
                        block_wg_col_prec);
                }
...
sycl::event MatrixMatrixOperationsMixed::symmetricMatrixMatrixDiagonal_optimizedCPU(
    ...
    sycl::event event = queue.submit([&](sycl::handler &h) {
        h.parallel_for(globalRange, [=](auto &id) {
            ...
            if (i >= j) {
                conf::fp_type value = read_element(ABytes, block_wg_diag_offset,
                                                   i * matrixBlockSize + j, block_wg_diag_prec);
                // perform update for lower triangle of the diagonal
#pragma clang loop vectorize(enable) unroll(enable)
                for (int k = 0; k < static_cast<int>(matrixBlockSize); ++k) {
                    // B_diag = B_diag - B_col * B_col^T
                    value = value - read_element(ABytes, block_wg_col_offset,
        ...
        h.parallel_for(kernelRange, [=](auto &nd_item) {
            ...
            for (int t = 0; t < sharedMemBlockCount; ++t) {
                ...
                for (int s = 0; s < valuesPerWorkItem_xy; ++s) {
                    local_tile_B[local_i + s * wgSize_xy][local_j] = read_element(
                        ABytes, blockOffset_B,
                ...
                // transposed block
                for (int s = 0; s < valuesPerWorkItem_xy; ++s) {
                    local_tile_C[local_i][local_j + s * wgSize_xy] = read_element(
                        ABytes, blockOffset_C,
            ...
            for (int ii = 0; ii < valuesPerWorkItem_xy; ++ii) {
                for (int jj = 0; jj < valuesPerWorkItem_xy; ++jj) {
                    tmp_val = read_element(ABytes, blockOffset_A,
                                           (i + ii) * matrixBlockSize + (j + jj), blockPrec_A) -
                              workItemTile[ii][jj];
                    write_element(ABytes, blockOffset_A, (i + ii) * matrixBlockSize + (j + jj),
                                  tmp_val, blockPrec_A);
                }
            }
        });
