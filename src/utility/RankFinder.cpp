...
int RankFinder::compute_rank_qr(std::vector<double> &A, int m, int n, double tol) {
...
int RankFinder::compute_block_rank(
    const std::vector<conf::fp_type, sycl::usm_allocator<conf::fp_type, sycl::usm::alloc::host>>
        &trainingInputData,
    std::size_t i_block, std::size_t j_block, std::size_t N, std::size_t matrixBlockSize,
    std::size_t nRegressors, double verticalLengthscale, double lengthscale, double noiseVariance) {
...
std::vector<int> RankFinder::compute_all_block_ranks(
    ...
    const std::vector<conf::fp_type, sycl::usm_allocator<conf::fp_type, sycl::usm::alloc::host>>
        &trainingInputData,
    std::size_t N, std::size_t matrixBlockSize, std::size_t nRegressors, double verticalLengthscale,
    double lengthscale, double noiseVariance) {
...
#pragma omp parallel for collapse(2) schedule(dynamic)
    for (std::size_t i_block = 0; i_block < blockCountXY; ++i_block) {
        for (std::size_t j_block = 0; j_block <= i_block; ++j_block) {
            ...
            int rank =
                compute_block_rank(trainingInputData, i_block, j_block, N, matrixBlockSize,
                                   nRegressors, verticalLengthscale, lengthscale, noiseVariance);
...
std::vector<int> RankFinder::compute_block_precisions(
    ...
    std::vector<int> blockRanks =
        compute_all_block_ranks(queue, trainingInputData, N, matrixBlockSize, nRegressors,
