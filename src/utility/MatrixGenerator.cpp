...
SymmetricMatrix MatrixGenerator::generateSPDMatrixStrictDiagonalDominant(sycl::queue &queue) {
    ...
    for (std::size_t block_i = 0; block_i < static_cast<std::size_t>(matrix.blockCountXY);
         ++block_i) {
                ...
                for (std::size_t i = 0; i < static_cast<std::size_t>(matrix.blockSize); ++i) {
                    for (std::size_t j = 0; j <= i; ++j) {
                        if (block_i * conf::matrixBlockSize + i < conf::N &&
                            ...
                            if (i == j) {
                                matrix.matrixData[blockStartIndex + i * conf::matrixBlockSize + j] =
                                ...
                                matrix.matrixData[blockStartIndex + j * conf::matrixBlockSize + i] =
                                    value;
                            }
                        }
                    }
...
SymmetricMatrix MatrixGenerator::generateSPDMatrix_optimized(std::string &path, sycl::queue &queue,
                                                             sycl::queue &queueGPU) {
    std::cout << "-- generating SPD matrix of size (optimized Kernel) " << conf::N << "x" << conf::N
              << std::endl;
    SymmetricMatrix matrix(conf::N, conf::matrixBlockSize, queueGPU);
...
void MatrixGenerator::generateTestKernelMatrix(std::string &path_train, std::string &path_test,
