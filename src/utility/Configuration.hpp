...
#ifdef USE_DOUBLE
typedef double
    fp_type; /// use double precision for all floating point operations
#else
typedef float
    fp_type; /// use single precision for all floating point operations
#endif

struct Configuration {
  int matrixBlockSize =
      128; /// Block size for storing the symmetric matrix in memory
  int workGroupSize = 128; /// Work-group size for matrix-vector operations
  int workGroupSizeVector =
      128; /// Work-group size for vector-vector operations
  ...
  double epsilon =
      1.0e-6; /// epsilon value for the termination of the cg algorithm
  int updateInterval =
      10; /// interval in which CPU/GPU distribution will be rebalanced
  double initialProportionGPU =
      0.5; /// initial proportion of work assigned to gpu
  ...
  double runtimeLBFactorCPU =
      1.2; /// factor that scales the CPU runtimes to influence the scheduling
           /// for lowest runtime
  double energyLBFactorCPU =
      0.8; /// factor that scales the CPU runtimes to influence the scheduling
           /// for power efficiency
  std::size_t blockUpdateThreshold =
      1; /// when block count change during re-balancing is equal or below this
  ...
  hws::sample_category sampleCategories = static_cast<hws::sample_category>(
      0b00000101);        /// enable power and general samples
  ...
  int workGroupSizeGEMM_xy =
      16; /// work-group size in x/y direction for GEMM kernels
  int minBlockCountCholesky = 3; /// minimum number of rows assigned to the GPU
                                 /// in the Cholesky decomposition
  int gpuOptimizationLevel =
      3; /// optimization level for GPU optimized matrix-matrix kernel (higher
         /// values for more optimized kernels)
  int cpuOptimizationLevel =
      2; /// optimization level for CPU optimized matrix-matrix kernel (higher
         /// values for more optimized kernels)

  bool printVerbose = false; /// enable/disable verbose output with detailed
                             /// timing on the console
  bool checkResult =
      false; /// enable/disable result check that outputs error of Ax - b

  bool trackCholeskySolveStep =
      true; /// hws library will track the solve step too if enabled

  bool unifiedAddressSpace =
      false; /// assume a unified address space for CPUs and GPUs.
...
inline int &workGroupSizeFinalScalarProduct =
    get().workGroupSizeFinalScalarProduct;
