 ...
 * Symmetric values in diagonal blocks are stored redundantly. Otherwise, only the lower triangle of
 * the matrix is stored.
 *
 * The block size can be set via the constructor.
 *
 * Blocks in the lower (blocked) triangle of the matrix are enumerated from top to bottom and from
 * left to right in the matrix. Internally blocks are stored after each other according to their ID.
